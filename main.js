...
//  adding the event listeners 
  ...
  multipleOptionsSelect.addEventListener("click", showSelectedOptions);
   ...
   // select elements from the multiple select on click - not ctrl+click
      ...
      for (let changingElement of changingElements) {
        ...
        if (checkBoxChecked) {
          ...
          elementsToChange = changingElementRow.querySelectorAll(
            ...
            const priceInputs = document.querySelectorAll(`[data-change='price'][data-col='${number}']`);
          ...
          if (!currentElement.hasAttribute("changed")) {
              if(priceToPromo.checked) {
                    ...
                    const priceToGet = currentElement.closest("tr").querySelector(`[data-change="price"][data-col="${number}"]`);
